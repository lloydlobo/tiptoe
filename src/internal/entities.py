...
class PhysicalEntity:
    def __init__(
        self, game: Game, entity_kind: pre.EntityKind, pos: pg.Vector2, size: pg.Vector2
    ) -> None:
        self.game = game
        ...
        self.anim_offset = pg.Vector2(
            -1, -1
        )  # | Workaround for padding used in animated sprites states like run jump
    ...
    def update(self, tilemap: Tilemap, movement: pg.Vector2 = pg.Vector2(0, 0)) -> bool:
        ...
        # Update velocity
        self.velocity.y = min(
            self._terminal_velocity_y, self.velocity.y + self._terminal_limiter_air_friction
        )
...
class Enemy(PhysicalEntity):
    def __init__(self, game: Game, pos: pg.Vector2, size: pg.Vector2) -> None:
        ...
        self._max_alert_time: Final = (
            60 * 2.5
        ) * 2  # aiming for alert for 5 seconds as if the enemy stops moving the agitation isn't shown for that time period?
        ...
        self.history_contact_with_player: deque[
            tuple[float, Literal['e-face-left', 'e-face-right'], tuple[str, str]]
        ] = deque(
            maxlen=pre.FPS_CAP * 2
        )  # _type:ignore
    ...
    def update(self, tilemap: Tilemap, movement: pg.Vector2 = pg.Vector2(0, 0)) -> bool:
        ...
        match (self.walking_timer > 0):
            case True:  # Movement via timer
                ...
                lookahead = pg.Vector2(
                    self.rect().centerx + lookahead_x, self.pos.y + self._lookahead_y
                )
                ...
                match solid_ahead, self.collisions.left, self.collisions.right:
                    ...
                    case (True, False, False):  # turn
                        ...
                        if (
                            self._alertness_enabled and self.alert_timer
                        ):  # Calculate moving average for smooth/erratic movement
                            if (
                                _tmp_disabling_as_they_may_shoot_opposite_side := 0
                            ) and _tmp_disabling_as_they_may_shoot_opposite_side:
                                avg_mvmt_x = 0.1 * round(
                                    10
                                    * sum(self.movement_history_x)
                                    / len(self.movement_history_x)
                            ...
                            extra_crazy = math.sin(self.alert_timer) * randint(
                                0, 2
                            )  # agitated little hops
                ...
                if not self.walking_timer:
                    dist_pe = pg.Vector2(
                        self.game.player.pos.x - self.pos.x, self.game.player.pos.y - self.pos.y
                    ...
                    if self._alertness_enabled and self.alert_timer:
                        ...
                        if (self.walking_timer <= self._max_alert_time) and (
                            random() < 0.01 * self.alert_boost_factor
    ...
    def render(self, surf: pg.SurfaceType, offset: tuple[int, int] = (0, 0)) -> None:
        ...
        if _tmp_enable_pretty_rays or (self._alertness_enabled and self.alert_timer):
            ...
            if 0 < hue < 100:
                ...
                if dist_btw_player_enemy.length() <= math.sqrt(
                    (
                        (self._lookahead_x**2) * pre.TILE_SIZE
                        + (self._lookahead_y**2) * pre.TILE_SIZE
                    )
                ):
                    if _tmp_enable_pretty_line_rays:
                        ...
                        if _tmp_enable_pretty_arc_rays:
                            ...
                            def manhattan_dist(
                                x1: pre.Number, y1: pre.Number, x2: pre.Number, y2: pre.Number
                            ) -> pre.Number:
                            ...
                            player_corner_l_dist = manhattan_dist(
                                *corner_l, *(self.game.player.pos)
                            )
                            player_corner_m_dist = manhattan_dist(
                                *corner_m, *(self.game.player.pos)
                            )
                            width = math.ceil(player_corner_l_dist + player_corner_m_dist)
                            if 1:
                                if 0:
                                    rect = line_rect.inflate(
                                        player_corner_l_dist, player_corner_m_dist
                                    )
                                else:
                                    distx = player_corner_l_dist * (
                                        1
                                        - 2
                                        * math.pi
                                        * 0.328
                                        * math.sin(width - (256 - hue))
                                        * 0.03
                                    )  # 0.03 balances cases of math.sin looping and returning large numbers
                                    disty = player_corner_m_dist * (
                                        1
                                        - 2
                                        * math.pi
                                        * 0.328
    ...
    def spawn_projectile_with_sparks(self):
        ...
        self.game.projectiles.append(
            pre.Projectile(
                pos=pg.Vector2(
                    self.rect().centerx + self.get_flip_dir() * SIZE_GUN[0], self.rect().centery
                ),
...
class Player(PhysicalEntity):
    ...
    def update(self, tilemap: Tilemap, movement: pg.Vector2 = pg.Vector2(0, 0)) -> bool:
        ...
        if not self.wall_slide:
            if self.air_time > self._max_air_time - 1:
                ...
                self.set_action(
                    Action.IDLE
                )  # Player IDLE state blends into the nearby color and can't be seen by enemies
        ...
        if abs(self.dash_time) > 50:  # at first ten frames of dash abs(60 -> 50)
            self.velocity.x = self._dash_thrust * (
                abs(self.dash_time) / self.dash_time
        ...
        self.velocity.x = (
            max(0, self.velocity.x - 0.1)
            if (self.velocity.x > 0)
            else min(0, self.velocity.x + 0.1)
        )
    ...
    def calculate_bezier_particle_radius(self) -> float:
        ...
        radius = (
            ((1 - t) ** 2 * 0) + (2 * t * (1 - t) * 1) + (t**2 * 3)
        )  # Calculate the radius based on the Bezier curve quadratic formula
        radius *= (
            pstar_radius * 0.5
        )  # Scale the radius based on the maximum dash time and the playersstar dash size
    ...
    def render(self, surf: pg.SurfaceType, offset: tuple[int, int] = (0, 0)) -> None:
        ...
        near_freefall_death = (
            self.air_time + self._max_air_time + pre.FPS_CAP // 4 >= self._air_time_freefall_death
        )  # ^ how to use this as walrus operator?
        if (not (abs(self.dash_time) <= self.dash_time_burst_2)) or near_freefall_death:
            ...
            if (
                near_freefall_death
                and (is_mid_air := not any(self.collisions.__dict__.values()))
                and is_mid_air
            ...
            if (
                not self.game.screen.get_rect().contains(self.rect())
                and self.air_time >= self._air_time_freefall_death
            ):  # dying
                self._drawcircle_starfn(
                    surface=surf,
                    center=(self.pos - offset),
