...
class Player(PhysicalEntity):
    ...
    def render(self, surf: pg.SurfaceType, offset: tuple[int, int] = (0, 0)) -> None:
        ...
        if (not (abs(self.dash_time) <= self.dash_time_burst_2)) or near_freefall_death:
            self._drawcircle_starfn(surface=surf, center=(self.pos - offset), radius=self.calculate_bezier_particle_radius() * 1.328)  # player is invincible and invisible
            if near_freefall_death and (is_mid_air := not any(self.collisions.__dict__.values())) and is_mid_air:  # freefall  # NOTE: is_mid_air is unreliable as it constantly flips on off since it listens to keyup and keydown movements
