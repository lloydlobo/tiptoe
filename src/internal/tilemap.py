...
class Tilemap:
    def __init__(self, game: Game | Editor, tile_size: int = pre.TILE_SIZE) -> None:
        ...
        self.offgrid_tiles: Set[TileItem] = set()
        self.tilemap: Dict[str, TileItem] = {}

        # Derived local like variables
        self.game_assets_tiles = self.game.assets.tiles
        # HACK(Lloyd): this can be an issue if screen is resized.
        self.dimensions: Final = pg.Vector2(self.game.display.get_rect().w, self.game.display.get_rect().h)
        ...
        self._locfmt_p_fn.__doc__ = (
            "This partial function takes x and y integers and serializes into json map_data dictionary keys."
        )
    ...
    def physics_rects_around(self, pos: tuple[int, int]) -> Iterable[pg.Rect]:
        size = self.tilesize
        return (
            self._pg_rect_p_fn(tile.pos.x * size, tile.pos.y * size, size, size)
    ...
    def extract(self, id_pairs: Sequence[Tuple[str, int]], keep: bool = False) -> List[TileItem]:
        ...
        for loc, tile in self.tilemap.items():
            if (tile.kind.value, tile.variant) in id_pairs:
                ...
                #     matches.append(tile.copy()) # convert to a copyable position obj if it is immutable
    ...
    def load(self, path: str | Path) -> None:
        ...
        map_w, map_h = self._calculate_loaded_level_map_dimension()
        # NOTE: adjust map size based on preset screen resolution sizes defined
        # in src/internal/prelude.py
        self.cur_level_map_dimension = (max(disp_w, map_w), max(disp_h, map_h))
    ...
    def offgrid_tiles_to_json(self) -> list[TileItemJSON]:
        return [
            TileItemJSON(kind=tile.kind.value, pos=tuple(tile.pos), variant=tile.variant) for tile in self.offgrid_tiles
        ]
    ...
    def offgrid_tiles_json_to_dataclass(data: Sequence[TileItemJSON]) -> Iterator[TileItem]:
        return (
            TileItem(kind=pre.TileKind(tile["kind"]), pos=pg.Vector2(tile["pos"]), variant=tile["variant"])
            for tile in data
        )
    ...
    def tilemap_json_to_dataclass(data: Mapping[str, TileItemJSON]) -> Iterator[tuple[str, TileItem]]:
        return (
            (key, TileItem(kind=pre.TileKind(tile["kind"]), pos=pg.Vector2(tile["pos"]), variant=tile["variant"]))
    ...
    def autotile(self) -> None:
        ...
        if (matrix_tiles := grouped_tiles.get("matrix")) and matrix_tiles:
        ...
        if (horz_tiles := grouped_tiles.get("horizontal")) and horz_tiles:
            for tile in horz_tiles:
                neighbors = set(
                    (x, y)
        ...
        if (vert_tiles := grouped_tiles.get("vertical")) and vert_tiles:
            for tile in vert_tiles:
                neighbors = set(
                    (x, y)
